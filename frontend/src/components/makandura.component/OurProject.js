...
const ProjectMakandura = () => {
  const [projectmakandura, setProjectMakandura] = useState([]);
  // console.log(events);
  console.log(projectmakandura);
  // const [status, setStatus] = useState(isChacked);

  const getProjectMakandura = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/projectmakandura/`);
      // console.log(response);
      //only status is true data will be shown
      setProjectMakandura(response.data.filter((item) => item.status === true)); //only status is true data will be shown
  ...
  return (
    <>
      <div className="container-fluid px-5 pb-5 pt-3 rounded-3 shadow bg-white mb-5">
        <h1 className="text-center pb-3 text-3xl">Makadura Project</h1>
        <HomeWrapper>
          <Slider {...settings} className={styles.card_container}>
            {projectmakandura.map((curElem) => {
              console.log(curElem,"test");
              console.log("heeeeeleleelle");
              return (
